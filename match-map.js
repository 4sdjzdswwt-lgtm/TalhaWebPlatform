...
let matchMapMySettings = { sharing:false, ghostMode:false, visibility:'mutual', excludedUids:{}, trackMode:'normal', msgApprovalOn:true };
...
(function injectMatchMapStyles(){
  ...
  .matchMapStyleRow{position:absolute;bottom:calc(env(safe-area-inset-bottom,0px) + 18px);left:0;right:0;display:flex;justify-content:center;gap:8px;z-index:2;pointer-events:none;}
  .matchMapStyleRow > *{pointer-events:auto;}
  .matchMapStyleChip{padding:8px 14px;border-radius:20px;background:var(--glass-bg);backdrop-filter:blur(10px);border:1px solid var(--glass-border);
    color:#fff;font-size:12.5px;font-weight:600;cursor:pointer;}
  .matchMapStyleChip.active{background:var(--gradient-vivid);}
    ...
    overlay.innerHTML = `
      ...
      <div class="matchMapTopBar">
        ...
        <div class="matchMapRightBtns">
          <button class="matchMapIconBtn" onclick="goToMyMatchLocation()" title="${escapeHtml(t('match_my_location'))}">
            <svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2.2"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/></svg>
          </button>
      ...
      <div class="matchMapStyleRow">
        <div class="matchMapStyleChip" data-style="satellite" onclick="setMatchMapStyle('satellite')">${escapeHtml(t('match_style_satellite'))}</div>
        <div class="matchMapStyleChip" data-style="dark" onclick="setMatchMapStyle('dark')">${escapeHtml(t('match_style_dark'))}</div>
        <div class="matchMapStyleChip" data-style="light" onclick="setMatchMapStyle('light')">${escapeHtml(t('match_style_light'))}</div>
      </div>
...
function initMatchMapInstance(){
  ...
  updateMatchStyleChipsUI();
}

function setMatchMapStyle(key){
  matchMapStyleKey = key;
  updateMatchStyleChipsUI();
  if(matchMap) matchMap.setStyle(MATCH_MAP_STYLES[key] || MATCH_MAP_STYLES.dark);
  matchMap && matchMap.once('style.load', ()=>{ applyMatchMapHolidayTheme(); renderAllMatchMarkers(matchMapLastCandidates || []); });
}
function updateMatchStyleChipsUI(){
  document.querySelectorAll('.matchMapStyleChip').forEach(chip=>{
    chip.classList.toggle('active', chip.dataset.style === matchMapStyleKey);
  });
}
  ...
  return Promise.all([
    ...
    Object.keys(all).forEach(uid=>{
      ...
      if(vis === 'mutual'){
        ...
        return; // bilinmeyen değer — güvenli taraf: gösterme
      }
...
function openMatchProfilePreview(uid, candidateArg){
  ...
  overlay.innerHTML = `
    <div class="followListSheet" style="max-height:88vh;">
      <div class="followListHead">
        <h3>${escapeHtml(profile.displayName || profile.username || '@kullanici')} ${typeof verifiedBadgeHtml==='function' ? verifiedBadgeHtml(profile.verifiedTier, 15) : ''}</h3>
        <button class="followListClose" onclick="closeMatchProfilePreview()">✕</button>
      </div>
...
function openMatchSettings(){
  ...
  const visOptions = [
    ...
    { key:'public', title:t('match_vis_public'), desc:t('match_vis_public_desc') }
  ];
  const trackOptions = [
  ...
  overlay.innerHTML = `
    <div class="followListSheet" style="max-height:92vh;">
      <div class="followListHead">
        <h3>⚙️ ${escapeHtml(t('match_settings_title'))}</h3>
        <button class="followListClose" onclick="closeMatchSettings()">✕</button>
      </div>
      <div class="followListBody" style="padding:0;">
        ...
        <div style="padding:14px 18px 4px;font-size:12.5px;font-weight:700;color:var(--muted);">${escapeHtml(t('match_tracking_mode_title'))}</div>
  ...
  document.body.appendChild(overlay);
  if(s.visibility === 'except') loadMatchExcludeChips();
}
...
function setMatchVisibility(key){
  ...
  if(key === 'except') loadMatchExcludeChips();
  fbDb.ref('userLocations/' + myUid + '/visibility').set(key).then(()=> refreshNearbyMatchUsers());
}

function loadMatchExcludeChips(){
  const myUid = fbAuth.currentUser.uid;
  const wrap = document.getElementById('matchExcludeChips');
  Promise.all([
    fbDb.ref('follows/' + myUid).once('value'),
    fbDb.ref('followers/' + myUid).once('value'),
    fbDb.ref('userLocations/' + myUid + '/excludedUids').once('value')
  ]).then(([followsSnap, followersSnap, excludedSnap])=>{
    ...
    const excluded = excludedSnap.val() || {};
    if(!wrap) return;
    ...
    Promise.all(mutualUids.map(uid=> fbDb.ref('users/' + uid).once('value').then(s=>({uid, data:s.val()||{}})))).then(results=>{
      wrap.innerHTML = results.map(r=>{
        const isExcluded = !!excluded[r.uid];
        const name = escapeHtml(r.data.username || r.data.displayName || '@kullanici');
        return `<div class="matchExcludeChip ${isExcluded?'excluded':''}" data-uid="${r.uid}" onclick="toggleMatchExcludeUid('${r.uid}')">${isExcluded ? escapeHtml(t('match_excluded_chip')) : escapeHtml(t('match_exclude_chip'))} · ${name}</div>`;
      }).join('');
    });
  });
}
function toggleMatchExcludeUid(uid){
  const myUid = fbAuth.currentUser.uid;
  const ref = fbDb.ref('userLocations/' + myUid + '/excludedUids/' + uid);
  ref.once('value').then(snap=>{
    const next = !snap.exists();
    (next ? ref.set(true) : ref.remove()).then(()=>{
      loadMatchExcludeChips();
      refreshNearbyMatchUsers();
    });
  });
}
