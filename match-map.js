...
/* Hex rengi verilen miktarda (yüzde, -100..100) açar/koyulaştırır — 3D küre
   efekti için üstte açık, altta koyu ton üretmede kullanılıyor. */
...
/* Konum noktalarını düz renkli daire yerine GERÇEK 3D görünümlü bir küre
   (top) olarak çiziyoruz: altında zemine düşen eliptik gölge, gövdede
   üstten aşağı radial-gradient (ışık üstten geliyor hissi) ve üstte
   parlak bir highlight — haritadaki 3D binalarla görsel bütünlük sağlar. */
function generateMatchDotImageData(hexColor, isMe){
  ...
  const r = MATCH_DOT_IMAGE_SIZE * 0.30;

  // 1) Zemine düşen gölge — topun altında, hafif ötelenmiş eliptik leke
  ctx.beginPath();
  ctx.ellipse(cx, cy + r * 1.15, r * 0.95, r * 0.34, 0, 0, Math.PI * 2);
  ctx.fillStyle = 'rgba(0,0,0,.4)';
  ctx.fill();

  // 2) Küre gövdesi — üst-sol'dan ışık alan radial gradient (3D top hissi)
  const grad = ctx.createRadialGradient(cx - r * 0.35, cy - r * 0.4, r * 0.1, cx, cy, r * 1.05);
  grad.addColorStop(0, matchShadeColor(hexColor, 55));
  grad.addColorStop(0.55, hexColor);
  grad.addColorStop(1, matchShadeColor(hexColor, -35));
  ctx.beginPath();
  ...
  ctx.fill();
  ctx.shadowBlur = 0;
  ctx.lineWidth = MATCH_DOT_IMAGE_SIZE * 0.045;
  ctx.strokeStyle = matchShadeColor(hexColor, -45);
  ctx.stroke();

  // 3) Üstte parlak highlight — cam/plastik top hissi tamamlar
  ctx.beginPath();
  ctx.ellipse(cx - r * 0.32, cy - r * 0.4, r * 0.32, r * 0.2, -0.5, 0, Math.PI * 2);
